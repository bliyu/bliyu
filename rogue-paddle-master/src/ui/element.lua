...
UiElement.new = function(build)
  ...
  for i = 1, table.maxn(build) do
    local v = build[i]
    if v then
      assert(Ui.is(v), 'passed an invalid id, to become a UiElements child')
  ...
  return Ui.add(e, {
    ...
    events = {
      ...
      remove = function(_)
        for _, child_id in ipairs(e._children) do
          Ui.remove(child_id)
        end
      end,

      layout = function(view, _parent, _propagate)
        for _, child in ipairs(e._children) do
          local child = Ui.get(child)
          assert(child, string.format('passed nil child to element: %s', view.state.name))
...
function UiElement.update(self, ctx, dt)
  ...
  if hover then
    for _, child_id in ipairs(self._children) do
      Ui.update(Ui.get(child_id), dt)
    end
  end
end
...
function UiElement:addChildren(children)
  ...
  for _, child_id in ipairs(children) do
    local child = Ui.get(child_id)
    table.insert(self._children, child_id)
...
function UiElement.position(self, view)
  ...
  for child_id = start_i, end_i, step do
    local child = Ui.get(self._children[child_id])
    assert(child, 'flex layout child is nil')
